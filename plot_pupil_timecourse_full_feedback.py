...
def plot_stat_comparison(group, train, comp1, comp2, comp1_stderr, comp2_stderr, p_val, p_fdr, p_mul, res_tfce, time, title, folder,
    ...
    ax.fill_between(time, y1 = p_mul+500, y2 = p_mul-500, where = ((p_val < 0.05)*(res_tfce==0)), facecolor = 'blue', alpha = 0.2, step = 'pre')
    ...
    path = f'/net/server/data/Archive/prob_learn/asmyasnikova83/Pupil/full_tmcrs/{group}/'
    os.makedirs(path, exist_ok = True)
    fig.set_size_inches((20, 10), forward=False)
    plt.savefig(path+'%s_vs_%s_%s' %(comp1_label, comp2_label, train) + '.png', transparent=False)
...
#all time points in the interval of interest
times_len = 4199
...
for j,subj in enumerate(subj_list):
    ...
    for run in runs:
        ...
        df_choice = df_name[df_name['trial_type4'] == parameter3]
        df_choice_tr = df_choice[df_choice['train'] == train]

        df_trained= df_choice_tr[df_choice_tr['feedback_prev'] == comp1]
        df_block1 = df_trained[df_trained['block'] == run]
        df_not_trained = df_choice_tr[df_choice_tr['feedback_prev'] == comp2]
        df_block2 = df_not_trained[df_not_trained['block'] == run]
        if len(df_block1) > 0:
            #over trials
            df_to_vstack = df_block1.iloc[:, 20:4219].mean(axis = 0)
            df_to_vstack = df_to_vstack[np.newaxis, np.newaxis, :]
            epochs_all_fb1 = np.vstack([epochs_all_fb1, df_to_vstack])
            print('df_to_vstack after', df_to_vstack.shape)

        if len(df_block2) > 0:
            df_to_vstack_cond2 = df_block2.iloc[:, 20:4219].mean(axis = 0)
...
if group == 'normals':
    comp1_label = comp1 + '_' + parameter3 + '_NT'
    comp2_label = comp2 + '_'+ parameter4 + '_NT'

if group == 'autists':
    comp1_label = comp1 + '_'+ parameter3 + '_AT'
    comp2_label = comp2 + '_' + parameter4 + '_AT'
...
time = np.arange(-1999,2200)
p_mul = 0

fig = plt.figure(figsize=(8, 5))

plot_stat_comparison(group=group, train=train, comp1=norisk_NT_mean, comp2=risk_NT_mean, comp1_stderr=norisk_NT_stderr, comp2_stderr=risk_NT_stderr, p_val=p_val, p_fdr=p_fdr,p_mul=p_mul, res_tfce=res_tfce, time=time, title=f'{comp1_label} vs {comp2_label} {train}',  folder='comparison',
