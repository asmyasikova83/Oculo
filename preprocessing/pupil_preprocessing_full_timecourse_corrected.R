...
full_table <- NULL
blink_table <- NULL
folder_out <- 'C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/asci/'

#nt <- c('P004','P019', 'P021', 'P022', 'P032', 'P034', 'P035', 'P039','P040', 'P044','P047','P048', 'P053',
#        'P055', 'P058', 'P059', 'P060', 'P061', 'P063', 'P064', 'P065', 'P066', 'P067', 'P001')
...
for(subject in nt){
    ...
    varname <- function(x){gsub('-','m',paste('V_',x*1-2000,sep=''))}
    ...
    avg_matrix <- matrix(ncol=5000,nrow=length(responses$time),data=0)
    for(trial_idx in 1:length(responses$time)){
      ...
      trial_avg <- matrix(trial_signal,ncol=5000)
      avg_matrix[trial_idx,] <- trial_avg
    }
  
    for(col_idx in 1:5000){
      responses[,n_col-5000+col_idx] <- avg_matrix[,col_idx]
    }
  

    full_table <- rbind(full_table,responses)
    fname <- paste0(folder_out, 'resp_merged_table.csv')
