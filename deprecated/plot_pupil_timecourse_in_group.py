...
#    ax.set_xlabel('time (ms)')
    ...
    path = '/net/server/data/Archive/prob_learn/asmyasnikova83/Pupil/bline_simple_aver/max/'
...
with open('/net/server/data/Archive/prob_learn/asmyasnikova83/Pupil/tables/pupil_table_trained_no_aver.csv', 'r') as inp, open('/net/server/data/Archive/prob_learn/asmyasnikova83/Pupil/edit.csv', 'w') as out:
    ...
    for row in csv.reader(inp):
        #if row[1] != "P034" and row[1] != "P048":
        #if row[1] != "P061" and row[1] != "P044" and row[1] != "P058":
        ...
        #f row[1] != "P040"  and row[1] != "P061" and row[1] != "P044" and row[1] != "P058":
            ...
            #for risk, norisk AT
            #if untrained and row[1] != "P326" and row[1] != "P328" and row[1] != "P304":

            #if   row[1] != "P316" and row[1] != "P333":
...
risk_negative_AT =AT[AT['trial_type4'] == 'prerisk']
...
#NT = df[df['group'] == 'normals']  
risk_negative_NT =NT[NT['trial_type4'] == 'norisk']
...
print(risk_negative_AT['trial_type4'])
...
risk_negative_NT_mean = risk_negative_NT.iloc[:,4:57].max(axis = 0)
risk_negative_NT = risk_negative_NT.iloc[:, 4:57]
risk_negative_AT_mean = risk_negative_AT.iloc[:, 4:57].max(axis = 0)
risk_negative_AT = risk_negative_AT.iloc[:, 4:57]

print('risk_negative_NT_mean shape', risk_negative_NT_mean.shape)
