...
path <- 'C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/' # path to files with p-vals
...
Tuk <- emmeans(m2, pairwise ~ feedback_prev|train|trial_type4|group  , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000,level = 0.99)
...
pic_1 <- ggplot(output_p1, aes(x=trial_type4, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  ...
  labs(x = 'Choice type', y = "Rel.pupil area 1000-2200ms emmeans",col="black")+
  ggtitle("not_trained neurotypical participants") +
...
pic_2 <- ggplot(output_p2, aes(x=trial_type4, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  ...
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
  labs(x = 'Choice type', y = "Rel.pupil area 1000-2200ms emmeans",col="black")+
  ggtitle("trained neurotypical participants") +
  scale_fill_discrete(name="Previous\nFeedback",
                      labels=c("Loss", "Gain"))
...
ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/Pupil_feedback_prev_AT.png',pn_at,width =  12, height = 5)

ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/Pupil_feedback_prev_NT.png',pn_nt,width =  12, height = 5)
