...
print(unique(df_large_group[group == 'autists' & train == 'not_trained']$fname))
...
title <- 'Нейротипичные испытуемые после обучения'
#title <- 'Испытуемые с РАС после обучения'
#title <- 'Choice_type'
p <-ggline(df_large_group, 'trial_type4', interval,
           color = 'feedback_prev',
           add = c("mean_se"),
           #palette = c('green','red'),
           order = c("norisk","risk"),
           ylab = 'Относительная площадь зрачка (1000-2200 мс)', xlab = "Choice type",
           size = 2.7, 
           point.size = 1.8,
           font.label = list(size = 16, color = "black"))+
  scale_x_discrete(name=title, labels = c("HP", 'LP'))+
  scale_fill_viridis(name = "Предыдущий исход", labels = c("Штраф", "Выигрыш"))+
