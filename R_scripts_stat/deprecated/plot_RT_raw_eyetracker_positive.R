...
df_large_neg <- df_large_group[feedback_prev == 'positive']
...
df_large_group_test <- df_large_group_test[trial_type %in% c('norisk','risk')]
######################################demographics#################################################
...
if (TRUE){
  Tuk <- NULL
  if (modes1== 'trained vs untrained'){
    #Tuk <- data.table(summary(emmeans(m, pairwise ~ trial_type|train|group, adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000))$contrasts)
    Tuk <- data.table(summary(emmeans(m, pairwise ~ train|trial_type|group, adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000))$contrasts)
   }
  if (modes1 != 'trained vs untrained'){
...
Tuk[,y.position:=y.position-0.25]
Tuk

choice_type = FALSE
choice_type_combined = TRUE
feedback_prev = FALSE
...
if (choice_type == TRUE){
    ...
    p <-ggline(dff_trained, 'trial_type', interval,
               add = c("mean_se"),
               order = c("norisk", "risk"),
               ylab = 'RT_log_raw', xlab = "Choice type",
               size = 1.5, 
               point.size = 1.8,
...
if (choice_type_combined == TRUE){
    ...
    dff_trained <- df_large_group_test[group == as.character(modes2)]
    title <- paste0(as.character(modes2))
    interval
    ...
    y_values_rew <- df_large_group_test[train == 'trained',
                                   mean(get(interval)),by='trial_type']
    #mean(get('RT_Z')),by='trial_type4']
    setnames(y_values_rew,'V1','y_values_rew')
    
    y_values_lose <- df_large_group_test[train  == 'not_trained',
                                    mean(get(interval)),by='trial_type']
    #mean(get('RT_Z')),by='trial_type4']
    ...
    y_values <- y_values[group == as.character(modes2)]
    #plot
    p <-ggline(dff_trained, 'trial_type', interval,
               color = 'train',
               add = c("mean_se"),
