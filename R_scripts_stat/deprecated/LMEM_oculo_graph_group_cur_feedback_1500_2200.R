...
pupil_sd <- df_large[blink==F][,sd(resp_1501_2200)]
df_large[,resp_flag:=!blink & abs(resp_1501_2200)<3*pupil_sd]
...
df_large_group <- df_large_group[!is.na(resp_1501_2200)]
...
m <- lmer(resp_1501_2200 ~ trial_type4*group*feedback_cur*train + (1|fname) + (1|block),df_large_group)
...
#Tuk
Tuk <- emmeans(m, pairwise ~ feedback_cur | train | group  , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000)
...
#Tuk1[,y.position:=y.position+0.3]
Tuk1

signif <- Tuk1

################################################AT_not_trained############################################
output_p1 <-output[group == 'autists'][train == 'not_trained']
Tuk1_p1 <- Tuk1[group == 'autists'][train == 'not_trained']

#signif_p3 
p1 <- ggplot(output_p1, aes(x=group, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_cur))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+ylim(-0.5, 0.5)+
  theme_classic((size = 20))+ #scale_x_discrete(labels=c("LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
  labs(x = '', y = "Pupil [Z] 1500-2200 ms",col="black")+
  ggtitle("ASD: no learning") +
  scale_fill_discrete(name="Current\nFeedback",
                      labels=c("Loss", "Gain"))
p1
pic1<- ggpar(p1,
             ylim = c(-0.8, 0.1),
...
pic1
################################################AT_trained############################################
output_p2 <-output[group == 'autists'][train == 'trained']
Tuk1_p3 <- Tuk1[group == 'autists'][train == 'trained']

#signif_p3 
p2 <- ggplot(output_p2, aes(x=group, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_cur))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+ylim(-0.5, 0.5)+
  theme_classic((size = 20))+ #scale_x_discrete(labels=c("LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
#####################normals not trained###########################
output_p3 <-output[group == 'normals'][train == 'not_trained']
#y_values_p2 <- y_values[group == 'normals'][train == 'not_trained']
Tuk1_p3 <- Tuk1[group == 'normals'][train == 'not_trained']
#signif_p2 <- signif[group == 'normals'][train == 'not_trained']

#signif_p2
p3 <- ggplot(output_p3, aes(x=group, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_cur))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+#ylim(-0.5, 0.5)+
  theme_classic((size = 20))+ #scale_x_discrete(labels=c("LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
##########################################NT_trained##########################################
#####################normals trained###########################
output_p4 <-output[group == 'normals'][train == 'trained']
#y_values_p2 <- y_values[group == 'normals'][train == 'trained']
Tuk1_p4 <- Tuk1[group == 'normals'][train == 'trained']
#signif_p2 <- signif[group == 'normals'][train == 'trained']

#signif_p2
p4 <- ggplot(output_p4, aes(x=group, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_cur))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+#ylim(-0.5, 0.5)+
  theme_classic((size = 20))+ #scale_x_discrete(labels=c("LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
pn_nt<- plot_grid(pic3,pic4+ggplot2::theme(axis.text.y = ggplot2::element_blank(),
                                                         axis.ticks.y = ggplot2::element_blank(),
                                                         axis.title.y = ggplot2::element_blank()), nrow = 1, align = "hv")

pn_nt

pn_at<- plot_grid(pic1,pic2+ggplot2::theme(axis.text.y = ggplot2::element_blank(),
                                                         axis.ticks.y = ggplot2::element_blank(),
                                                         axis.title.y = ggplot2::element_blank()), nrow = 1, align = "hv")
...
ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/feedback_21_10/Pupil_feedback_curr_AT.png',pn_at,width =  12, height = 6)

ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/feedback_21_10/Pupil_feedback_curr_NT.png',pn_nt,width =  12, height = 6)
