...
if (modes[t1] != 'trained vs untrained'){
  ...
  m <- lmer(get(interval) ~ trial_type*feedback_prev*group + (1|fname) + (1|block), data = df_large_group)
...
if (FALSE){
  ...
  if (modes1 != 'trained vs untrained'){
    Tuk <- data.table(summary(emmeans(m2, pairwise ~ feedback_prev|train|trial_type|group, adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000))$contrasts)
...
if (choice_type_group == TRUE){
    ...
    dff_trained <- df_large_group[group == 'autists']
    dff_trained$train
