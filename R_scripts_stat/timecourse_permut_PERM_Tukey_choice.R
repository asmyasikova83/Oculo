...
for (j in cols){
  m <- lmp(get(j) ~ trial_type4, data = temp,  perm="Prob", maxIter = 5000, contrasts = list(a = "trial_type4") )
  
  phc <- posthoc_Pairwise(Model = m, Fixed_Factor = c("trial_type4"), 
                          P_adj = "tukey" )
  
  Tuk <- as.data.table(phc$contrasts)
  Tuk[,p.value:=format(p.value, digits = 3)]
  
  an <- data.table()
  an$interval <- j
  an$contrast <- Tuk$contrast
  an$p_val <- Tuk$p.value
...
p_vals <- p_vals[, trial_type_fdr:=p.adjust(`p_val`, method = 'fdr')] 
length(p_vals)
p_vals

write.csv(p_vals,paste0(out_path, "p_vals_factor_significance_pupil_permut_Tuk_at_trained_choice.csv"))