...
out_path <- 'C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_RT_Z/' #path to save pictures and tables
...
df_large_group <- df_large_group[trial_type4 %in% c('repetitive_risk','norisk')]
...
if (feedback_prev == TRUE){
  ...
  Tuk1 <- Tuk[group == as.character(modes[t2])]
  Tuk1
  title <- paste0(as.character(modes[t1]),'_', as.character(modes[t2]),'_by_fbprev')
