...
out_path <- paste0(path,'presentations_pics_posters/article_at_nt/')
...
#for singkLP repetLP
Tuk <- emmeans(m, pairwise ~ feedback_prev| trial_type4 | train   , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000)
...
##############################################################
#Tuk_pos <- emmeans(m, pairwise ~ train|feedback_prev|group  , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000,level = 0.99)
...
sequence <-data.table(train=c("not_trained","trained"),number=c(1,2))

y_values_rew <- df_large_group[feedback_prev == 'positive',
                               mean(get('resp_1001_2200')),by='train']
...
y_values <- merge(y_values_lose,y_values_rew,by='train')
y_values <- merge(y_values,sequence,by='train')
y_values[,y_max:=max(y_values_lose,y_values_rew),by=train]
y_values <- merge(y_values,signif,by='train')


#####################not trained###########################
output_p1 <-output2[train == 'not_trained']
output_p1[train == 'not_trained']
y_values_p1 <- y_values[train == 'not_trained']
Tuk1_p1 <- Tuk1[train == 'not_trained']
signif_p1 <- signif[train == 'not_trained']
...
pic_1 <- ggplot(output_p1, aes(x=trial_type4, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  ...
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+#ylim(-0.5, 0.5)+
  ...
  labs(x = '', y = "Rel.pupil area 1000-2200 ms",col="black")+
  ggtitle("NT: no learning") +
  scale_fill_discrete(name="Previous\nFeedback",
                      labels=c("Loss", "Gain"))
pic_1
pic1_signif <- pic_1+  geom_signif(y_position=c(y_values_p1$y_max+0.9),
                                   xmin=c(y_values_p1$number+0.5), xmax=c(y_values_p1$number+1.5),
...
pic1_signif
#####################trained###########################
output_p2 <-output2[train == 'trained']
y_values_p2 <- y_values[train == 'trained']
Tuk1_p2 <- Tuk1[train == 'trained']
signif_p2 <- signif[train == 'trained']
...
pic_2 <- ggplot(output_p2, aes(x=trial_type4, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  ...
  scale_fill_discrete(name="Previous\nFeedback",
                      labels=c("Loss", "Gain"))
pic_2
pic2_signif <- pic_2+  geom_signif(y_position=c(y_values_p2$y_max+0.9),
                                   xmin=c(y_values_p2$number-0.5), xmax=c(y_values_p2$number+0.5),
...
#pic2_signif
#####################autists not trained###########################
#output_p4 <-output2[train == 'not_trained']
#y_values_p4 <- y_values[train == 'not_trained']
#Tuk1_p4 <- Tuk1[train == 'not_trained']
#signif_p4 <- signif[train == 'not_trained']
...
#  theme_classic((size = 20))+ scale_x_discrete(labels=c('HP', 'LP'))+
#  geom_point(position=position_dodge(0.05)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
#  labs(x = '', y = "Rel.pupil area 1500-2200 ms",col="black")+
#  ggtitle("AT: no learning") +
#  scale_fill_discrete(name="Previous\nFeedback",
#                      labels=c("Loss", "Gain")) 
#pic_4
#pic4_signif <- pic_4 + geom_signif(y_position=c(y_values_p4$y_max+0.17),
#                                   xmin=c(y_values_p4$number), xmax=c(y_values_p4$number+0.4),
...
#pic4_signif
#####################autists trained###########################
#output_p3 <-output2[train == 'trained']
#y_values_p3 <- y_values[train == 'trained']
#Tuk1_p3 <- Tuk1[train == 'trained']
#signif_p3 <- signif[train == 'trained']
...
#                      labels=c("Loss", "Gain"))
#y_values_p3$number
#pic3_signif <- pic_3 + geom_signif(y_position=c(y_values_p3$y_max +0.7),
#                                   xmin=c(y_values_p3$number-0.5), xmax=c(y_values_p3$number+0.5),
...
#ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/presentations_pics_posters/article_at_nt/feedback_21_10/Pupil_feedback_prev_AT_by_group_separately_1000_2200ms.png',pn_at,width =  12, height = 6)

ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/presentations_pics_posters/article_at_nt/feedback_21_10/Pupil_feedback_prev_NT_by_group_separately_1000_2200ms.png',pn_nt,width =  12, height = 6)
