...
df_large[prev_risk==0 & risk==1 & next_risk==0, trial_type2:= 'risk']
df_large[prev_risk==0 & risk==0 & next_risk==0, trial_type2:= 'norisk']
...
df_large_group <- df_large_group[trial_type2 %in% c('norisk','risk')]
unique(df_large_group$trial_type2)
unique(df_large_group$train)
################################LMM##########################
m <- lmer(resp_1001_2200 ~ trial_type2*group*feedback_prev*train + (1|fname) + (1|block),df_large_group)
...
Tuk <- emmeans(m, pairwise ~ feedback_prev|train|trial_type2|group  , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000,level = 0.99)
...
thr <- max(df_large_group[, mean(get(interval)) + sterr(get(interval)), by=trial_type2]$V1) #
thr <- thr-0.15
if (n>1){
...
Tuk_pos <- emmeans(m, pairwise ~ train|trial_type2|feedback_prev|group  , adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000,level = 0.99)
...
Tuk1_pos
n_pos <- Tuk1_pos[!is.na(p_significant), .N]
n_pos
thr <- max(df_large_group[, mean(get(interval)) + sterr(get(interval)), by=trial_type2]$V1) #
thr <- thr-0.15
if (n_pos>1){
  Tuk1_pos <- Tuk1_pos[!is.na(p_significant), y.position := seq((thr+0.01), (thr+0.3), 0.29/(n-1))]
} else {
  Tuk1_pos <- Tuk1[!is.na(p_significant), y.position := thr+0.36]
}
...
sequence <-data.table(trial_type2=c("norisk","risk"),number=c(1,2))
...
pic_1 <- ggplot(output_p1, aes(x=trial_type2, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  ...
  labs(x = 'Choice type', y = "Rel.puil area 1001-2200 ms",col="black")+
...
pic_2 <- ggplot(output_p2, aes(x=trial_type2, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+
  theme_classic((size = 20))+ scale_x_discrete(labels=c("HP", "LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
pic_4 <- ggplot(output_p4, aes(x=trial_type2, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+
  theme_classic((size = 20))+ scale_x_discrete(labels=c("HP", "LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
pic_3 <- ggplot(output_p3, aes(x=trial_type2, y=emmean, ymin=emmean-SE, ymax = emmean+SE, fill = feedback_prev))+
  geom_col(position = "dodge",colour =  c('black')) +
  geom_hline(yintercept= 0.0, linetype='dashed', col = 'black', size = 1.5)+
  theme_classic((size = 20))+ scale_x_discrete(labels=c("HP", "LP"))+
  geom_point(position=position_dodge(0.09)) + geom_errorbar(width = 0.2,  position=position_dodge(0.9), size=0.6)+
...
ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/feedback_21_10/Pupil_feedback_prev_AT_by_choice.png',pn_at,width =  12, height = 5)

ggsave('C:/Users/trosh/OneDrive/jobs_Miasnikova/Oculo/pics_articles/article_at_nt/feedback_21_10/Pupil_feedback_prev_NT_by_choice.png',pn_nt,width =  12, height = 5)
