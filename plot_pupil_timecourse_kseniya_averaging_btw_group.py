...
def plot_stat_comparison(comp1, comp2, comp1_stderr, comp2_stderr, p_val, p_fdr, p_mul, time, title, folder,
    ...
    path = f'/net/server/data/Archive/prob_learn/asmyasnikova83/Pupil/long_timecourse/betw_group/'
...
if comp1 == 'prerisk':
    subj_list = ['P001', 'P004', 'P019', 'P021', 'P022', 'P032', 'P035', 'P039', 'P040', 'P047', 'P053', 'P055', 'P058', 'P059', 'P061', 'P063', 'P064', 'P065', 'P301', 'P304', 'P307', 'P312', 'P313', 'P314', 'P320', 'P321', 'P322', 'P324', 'P326', 'P327', 'P328', 'P329', 'P334', 'P335', 'P338', 'P341']
if comp1 == 'risk':
...
print('subj_list N-1', subj_list[N-1])
print('subj_list 2*N-1', subj_list[2*N-1])

comp1_label = comp1 + '_AT'
comp2_label = comp1 + '_NT'

risk_NT = contr[0:N-1, 0, :]
risk_AT = contr[N:2*N-1, 1, :]

risk_NT_mean = risk_NT.mean(axis=0)
risk_AT_mean = risk_AT.mean(axis=0)

risk_NT_mean = np.squeeze(risk_NT_mean, axis=0)
risk_AT_mean = np.squeeze(risk_AT_mean, axis=0)

risk_NT_stderr = scipy.stats.sem(contr[0:N-1, 0, :], axis=0)
risk_AT_stderr = scipy.stats.sem(contr[N:2*N-1, 1, :], axis=0)

risk_NT_stderr = np.squeeze(risk_NT_stderr, axis=0)
risk_AT_stderr = np.squeeze(risk_AT_stderr, axis=0)


t_stat, p_val = stats.ttest_ind(risk_NT, risk_AT, axis=0)
...
plot_stat_comparison(comp1=risk_AT_mean, comp2=risk_NT_mean, comp1_stderr=risk_AT_stderr, comp2_stderr=risk_NT_stderr, p_val=p_val, p_fdr=p_fdr,p_mul=p_mul, time=time, title=f'{comp1_label} vs {comp2_label}, {N} subjects',  folder='comparison',
                         comp1_label=comp1_label, comp2_label=comp2_label, comp1_color='magenta', comp2_color='orange')
