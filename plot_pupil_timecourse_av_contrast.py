...
def plot_stat_comparison(Tuk, show_contrast, group, train, comp1, comp2, comp1_stderr, comp2_stderr, contrast, p_val, p_fdr, p_mul, res_tfce, time, title, folder,
    ...
    ax.set_ylim(p_mul-1.15, p_mul+1.15)
...
#contr = np.zeros((len(23), 2, 1, times_len[1]))
...
#norisk_NT = df_norisk_normals.iloc[:, 66:115].mean(axis = 0).mean(axis=0)
norisk_NT_mean = df_norisk_normals.iloc[:, 56:110].mean(axis = 0)
#print('shape norisk_NT', norisk_NT.shape)

risk_NT = df_risk_normals.iloc[:, 56:110]
#print('shape risk_NT', risk_NT.shape)

#to get the overall figure representing mean in the interval
#risk_NT = df_risk_normals.iloc[:, 66:115].mean(axis = 0).mean(axis=0)
